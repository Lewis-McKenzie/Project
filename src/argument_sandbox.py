...
def main() -> None:
    ...
    acd_model = Loader.load_model(MODEL_PATH, "acd_rest_model")
    polarity_model = Loader.load_model(MODEL_PATH, "sp_rest_model")
    ...
    text_with_categories = [f"{category} {test_df.text[i+CUT_START]}" for i, categories in enumerate(predicted_categories[CUT_START:CUT_END]) for category in categories]
